...
function activeTabOf(wd) {
    try { return wd.tabs.tabMap.get(wd.tabs.activeTabIndex) || null; } catch { return null; }
}
    ...
    for (const w of wm.getAllWindows()) {
        ...
        w.tabs.tabMap.forEach((tab, index) => {
            try {
                ...
                if (privateSession ? wc.session !== privateSession : tab.isPrivate) return;
                if (new URL(wc.getURL()).origin !== origin) return;
                w.tabs.reload(index);
            } catch {}
        });
    }
...
function register(ipcMain, { wm }) {
    ...
    ipcMain.handle('open-site-info', async (e, anchor) => {
        ...
        const info: any = computeInfo(activeUrlOf(wd), sitePermissions);
    ...
    ipcMain.handle('site-permission-set', (e, name, value) => {
        ...
        if (!info || !info.origin) return true;
        ...
        if (info.private) {
            if (wd.siteInfoSession) permissionPrompt.setOverride(wd.siteInfoSession, info.origin, name, value);
        } else {
    ...
    ipcMain.handle('site-clear-data', async (e) => {
        ...
        if (info && info.origin) {
            ...
            try { await sess.clearStorageData({ origin: info.origin }); return true; } catch {}
        }
        return false;
    });

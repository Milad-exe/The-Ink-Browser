...
async function ensurePanel(wd) {
    if (wd.extensionsPanel) {
        if (wd.extensionsPanelReady) await wd.extensionsPanelReady;
    ...
    wd.extensionsPanelReady = new Promise<void>(res => view.webContents.once('did-finish-load', () => res()));
    await wd.extensionsPanelReady;
    return view;
}

function hidePanel(wd) {
    if (!wd?.extensionsPanel) return false;
    try {
        wd.extensionsPanel.setVisible(false);
        wd.extensionsPanelOpen = false;
        try { wd.window.webContents.send('extensions-panel-closed'); } catch {}
        return true;
    } catch { return false; }
}
...
function register(ipcMain, { wm }) {
    ...
    extensions.onChanged(() => {
        for (const wd of wm.getAllWindows()) {
            ...
            if (wd.extensionsPanel) {
                try { wd.extensionsPanel.webContents.send('extensions-data', listWithPinned()); } catch {}
            }
        }
    });
    ...
    ipcMain.handle('extensions-open-store', (_e) => {
        ...
        for (const [idx, url] of wd.tabs.tabUrls) {
            if (typeof url === 'string' && url.includes('chromewebstore.google.com')) {
                try { wd.tabs.showTab(idx); wd.window.focus(); } catch {}
                return true;
            }
        }
    ...
    ipcMain.handle('extensions-set-pinned', (_e, id, pinned) => {
        ...
        for (const wd of wm.getAllWindows()) {
            try { wd.window.webContents.send('ext-pinned-changed', map); } catch {}
            if (wd.extensionsPanel) {
                try { wd.extensionsPanel.webContents.send('extensions-data', listWithPinned()); } catch {}
            }
        }
    ...
    ipcMain.handle('extensions-activate', (_e, id) => {
        ...
        if (!wd) return false;
        hidePanel(wd);
        try { wd.window.webContents.send('ext-activate-action', id); } catch {}
        return true;
    });
    ...
    ipcMain.handle('extensions-panel-toggle', async (_e, anchor) => {
        ...
        if (wd.extensionsPanelOpen) { hidePanel(wd); return false; }
        ...
        try {
            ...
            try { wd.window.contentView.removeChildView(view); } catch {}
            try { wd.window.contentView.addChildView(view); } catch {}

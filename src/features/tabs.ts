import { WebContentsView, BrowserWindow, Menu, shell, app } from 'electron';
...
import FindDialogManager from './find-dialog';
import focusMode from './focus-mode';
import { isSafeExternal } from './url-security';
import { READERABLE_JS, EXTRACT_JS, PIP_JS } from './reader';
import extensions from './extensions';
import * as miniPlayer from './mini-player';
import * as privateSessions from './private-session';
...
export interface TabView extends WebContentsView {
    lazyLoaded?: boolean;       // false while a restored ("lazy") tab hasn't loaded its URL yet
    lazyTitle?: string;         // placeholder title shown before a lazy tab first loads
    mutedUntilShown?: boolean;  // background-created tab stays muted until first shown
    isPrivate?: boolean;        // tab runs in its own in-memory private session
    privateSession?: any;       // that per-tab session (see features/private-session.ts)
    slept?: boolean;            // renderer was crashed on purpose to free memory ("tab sleeping")
    hasPlayingMedia?: boolean;  // media actively playing (drives tab-strip indicator + sleep veto)
    needsReloadForFocusMode?: boolean;   // focus-mode toggle needs a reload to take effect
    needsReloadForShortform?: boolean;
    setNavigatingProgrammatically?: (v: boolean) => void;
}
...
class Tabs {
    ...
    history: History;
    persistence: any;                     // Persistence — injected by WindowManager
    navigationHistory: NavigationHistory;
    findDialog: any;                      // FindDialog for this window
    shortcuts: any;                       // Shortcuts — set later via setShortcuts()
    windowManager: any;                   // WindowManager — set later via setWindowManager()

    // ── Tab bookkeeping ───────────────────────────────────────────────────
    tabMap: Map<number, TabView>;         // tabIndex → live view
    tabUrls: Map<number, string>;         // tabIndex → last committed URL
    tabOrder: number[];                   // visual order of tab indices
    tabLastActive: Map<number, number>;   // tabIndex → Date.now() of last activation
    activeTabIndex: number;
    nextTabIndex: number;                 // monotonically increasing id source
    pinnedTabs: Set<number>;
    privateTabs: Set<number>;
    isPrivateWindow: boolean;
    closedTabHistory: Array<{ url: string; title: string }>; // stack for "Reopen Closed Tab"

    // ── Window close / fullscreen state machines ──────────────────────────
    allowClose: boolean;
    closePreventionActive: boolean;
    isHtmlFullScreen: boolean;            // page called requestFullscreen()
    htmlFullScreenRequested: boolean;
    userFullScreenActive: boolean;        // user pressed F11 / green button

    // ── Reader mode ───────────────────────────────────────────────────────
    readerMode: Set<number>;              // tab indices currently showing the reader view
    readerArticles: Map<number, any>;     // tabIndex → extracted article
    readerOriginal: Map<number, string>;  // tabIndex → original URL to restore on exit

    // ── Timers / transient ────────────────────────────────────────────────
    bookmarkBarHeight: number;
    saveTimer: ReturnType<typeof setTimeout> | null;
    _sleepTimer: ReturnType<typeof setInterval>;
    _pendingPrevActive: number | null;    // tab to reactivate after a background-tab close

    constructor(mainWindow: BrowserWindow, History, Persistence, options: any = {}) {

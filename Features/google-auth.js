 ...
 *
 * The caller must have registered the protocol and set up handleProtocolCallback
 * via app.on('open-url') / app.on('second-instance') in main.js.
 */
function loginWithGoogle(clientId, clientSecret, scope = 'email profile') {
    return new Promise((resolve, reject) => {
        // Cancel any stale in-flight flow
 ...
 * (via app 'open-url' on macOS or 'second-instance' argv on Windows/Linux).
 */
function handleProtocolCallback(url) {
    if (!_resolve) return; // No flow in progress

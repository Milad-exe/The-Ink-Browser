...
const EXTRACT_JS = `(() => {
  try {
    ...
    root.querySelectorAll('a[href]').forEach(a => { try { a.href = new URL(a.getAttribute('href'), location.href).href; a.target='_top'; } catch (_) { /* runs in the page — no main-process log here */ } });
    root.querySelectorAll('img[src]').forEach(i => { try { i.src = new URL(i.getAttribute('src'), location.href).href; } catch (_) { /* runs in the page — no main-process log here */ } });

...
class SitePermissions extends EventEmitter {
    MANAGED; // permissions this store manages
    file; // encrypted store path (userData)
    data; // { perms: { origin → { perm → 'allow'|'deny' } }, sitesOff }
    loaded;
    _anyOff; // fast path: any per-site protection toggles off?
    constructor() {
        super();
        this.file = null;
        this.data = { perms: {}, sitesOff: {} };
        this.loaded = false;
    ...
    set(origin, name, value) {
        this._load();
        if (!origin || !MANAGED_NAMES.has(name))
            return;
        if (!this.data.perms[origin])
            this.data.perms[origin] = {};
        if (value === 'allow' || value === 'block')
            this.data.perms[origin][name] = value;
        else
            delete this.data.perms[origin][name]; // 'ask' / anything else clears it
        if (Object.keys(this.data.perms[origin]).length === 0)
            delete this.data.perms[origin];
        this._save();
    ...
    setProtection(site, off) {
        ...
        if (!site)
            return;
        if (off)
            this.data.sitesOff[site] = true;
        else
            delete this.data.sitesOff[site];
        this._anyOff = Object.keys(this.data.sitesOff).length > 0;
        this._save();
    }
}

...
> contextBridge scripts, one per view type — each page gets only the IPC
> surface it needs. Every bridge exposed here must be declared in
> `src/types/api.d.ts` so renderer TypeScript knows about it.
...
Legacy stub — no longer loaded by any window or `WebContentsView`. The Bruno panel uses `preload/preload.ts`, which exposes the full `window.bruno` bridge. This file is kept for reference only.

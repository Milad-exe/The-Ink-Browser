...
(() => {
    ...
    function renderBackground() {
        ...
        const glyphFor = (lum) => { if (lum < 0.055)
            return null; for (const [lim, g] of RAMP)
            if (lum < lim)
                return g; return 'W'; };
        ...
        for (let i = 0; i < 80; i++) {
            ...
            if (Math.hypot(c - pc, (r - pr) * (CH / CW)) < 8)
                continue;
        ...
        const polarisLum = (c, r) => {
            ...
            if (r === pr)
                lum = Math.max(lum, 0.75 * Math.max(0, 1 - Math.abs(dc) / 6));
            if (c === pc)
                lum = Math.max(lum, 0.75 * Math.max(0, 1 - Math.abs(dr) / 5));
        ...
        for (let r = 0; r < rows; r++) {
            for (let cc = 0; cc < cols; cc++) {
                ...
                if (u < -0.01 || v < -0.01)
                    continue; // outside the 70% region

...
(() => {
  ...
  <path d="M1 1l3 3.5L1 8"/>
    ...
    function isBookmarkDragEvent(e) {
        ...
        if (!dt)
            return false;
        if (Array.from(dt.types || []).includes('text/plain'))
            return true;
        return !!getDragIdFromEvent(e);
    }
    function ensureRaised() {
        const now = Date.now();
        if (now - lastRaiseAt < 120)
            return;
        lastRaiseAt = now;
    ...
    window.folderDropdown.onRefreshPanel(({ folderId, children, renameId }) => {
        ...
        for (let i = 0; i < backStack.length; i++) {
            if (backStack[i]?.id === folderId)
                backStack[i] = { ...backStack[i], children };
        }
    ...
    function buildList(folder) {
        ...
        list.addEventListener('dragover', (e) => {
            if (e.target.closest('.item[data-id]'))
                return;
            if (!isBookmarkDragEvent(e))
                return;
            clearBackHoverTimer();
    ...
    function buildBackButton(parentTitle) {
        ...
        btn.addEventListener('click', () => { if (!dragId)
            clickBack(); });
        ...
        btn.addEventListener('drop', async (e) => {
            ...
            if (!parentFolderId || srcId === parentFolderId)
                return;
            resetDragState();
    ...
    function buildFolderItem(btn, entry) {
        ...
        btn.addEventListener('click', () => { if (!dragId && !renamingId)
            clickInto(entry); });
    }
    function buildBookmarkItem(btn, entry) {
        ...
        btn.addEventListener('mouseup', (e) => {
            if (dragId || renamingId || e.button === 2)
                return;
    ...
    function attachItemDragHandlers(btn, entry, folderId, list) {
        ...
        btn.addEventListener('dragenter', (e) => {
            const srcId = getDragIdFromEvent(e);
            if (!isBookmarkDragEvent(e) || srcId === entry.id)
                return;
            e.preventDefault();
            ensureRaised();
            armHoverTarget();
        ...
        btn.addEventListener('drop', async (e) => {
            ...
            if (entry.type === 'folder') {
                ...
                const ids = Array.from(document.querySelector('.items-list')?.querySelectorAll('.item[data-id]') || []).map(el => el.dataset.id).filter(id => id !== srcId);
